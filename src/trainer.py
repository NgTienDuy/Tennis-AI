...
class Trainer:
    ...
    def train(self, epochs=1):
            ...
            for phase in ['train', 'valid']:
                ...
                if phase == 'valid':
                    ...
                    if epoch % 2 == 1:
                        ...
                        torch.save(saved_state, './models/' + self.saved_state_name + '_epoch_' + str(epoch))
        ...
        saved_state = dict(model_state=self.model.state_dict(), train_loss=train_loss, train_acc=train_acc,
                           valid_loss=valid_loss, valid_acc=valid_acc)
        torch.save(saved_state, './models/' + self.saved_state_name)
        print(f'*** Saved checkpoint ***')
...
def get_confusion_matrix(model_saved_state, dl=None):
    ...
    saved_state = torch.load('./models/' + model_saved_state, map_location='cpu')
