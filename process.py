...
def create_top_view(court_detector, detection_model):
    ...
    save_path = os.path.join(r'.\outputs\logs', 'top_view.avi')
    out = cv2.VideoWriter(save_path, cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 30, (v_width, v_height))
...
def video_process(video_path, show_video=False, include_video=True,
                  ...
                  output_file='output', output_folder=r'.\outputs\videos',
    ...
    ball_detector = BallDetector('tracknet_weights_2_classes.pth', out_channels=2)

    # Load videos from videos path
    video = cv2.VideoCapture(video_path)
    if not video.isOpened():
        print(f"LỖI NGHIÊM TRỌNG: Không thể mở video tại đường dẫn: {video_path}")
        return
    ...
    if total_time > 0:
        ...
        print('\nKhông xử lý được frame nào (total_time = 0). Kiểm tra lại đường dẫn video.')
...
def main():
    s = time.time()
    video_path = r'.\data\raw\videos\video_input8.mp4' 
    ...
    video_process(video_path=video_path, show_video=True, stickman=True, stickman_box=False, smoothing=True,
